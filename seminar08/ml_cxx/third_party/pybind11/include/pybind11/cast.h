...
PYBIND11_NOINLINE inline void all_type_info_populate(PyTypeObject *t, std::vector<type_info *> &bases) {
    ...
    for (size_t i = 0; i < check.size(); i++) {
        ...
        if (it != type_dict.end()) {
            // We found a cache entry for it, so it's either src-registered or has pre-computed
            // src bases, but we have to make sure we haven't already seen the type(s) before: we
...
// Calling cast() on an rvalue calls src::cast with the object rvalue, which does:
